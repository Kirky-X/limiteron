...
use crate::config::types::{FlowControlConfig, GlobalConfig, LimiterConfig, Matcher, Rule};
...
impl ConfigSecurityValidator {
    ...
    fn validate_window_size(
        ...
        if let Err(error) = crate::config::types::parse_window_size(window_size) {
            report.add_warning(format!(
                "规则[{}]限流器[{}]的窗口大小格式无效: {}",
...
mod tests {
    use super::*;
    use crate::config::types::{GlobalConfig, LimiterConfig, Matcher, Rule};
