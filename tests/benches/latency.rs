...
use limiteron::{
    limiters::{FixedWindowLimiter, SlidingWindowLimiter, TokenBucketLimiter},
    l2_cache::L2Cache,
};
...
fn bench_sliding_window_latency(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();
    let limiter = SlidingWindowLimiter::new(Duration::from_secs(1), 1000);
...
fn bench_fixed_window_latency(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();
    let limiter = FixedWindowLimiter::new(Duration::from_secs(1), 1000);
...
fn bench_l2_cache_hit_latency(c: &mut Criterion) {
    ...
    // 预热缓存
    rt.block_on(async {
        cache.set("hot_key", "hot_value", Some(Duration::from_secs(60))).await;
    });
...
fn bench_window_size_latency(c: &mut Criterion) {
    ...
    for (name, window_size) in window_sizes {
        let limiter = SlidingWindowLimiter::new(window_size, 1000);
