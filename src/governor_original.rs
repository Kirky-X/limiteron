...
pub struct Governor {
    ...
    /// 封禁管理器
    /// 负责管理用户、IP、MAC等目标的封禁状态
    ban_manager: Arc<BanManager>,
    
    /// 并行封禁检查器
    /// 高性能的并行封禁检查器，支持多目标并发验证
    /// 使用独立的模块减少 governor.rs 的复杂度
...
impl Governor {
    ...
    async fn initialize_decision_chain(&self) -> Result<(), FlowGuardError> {
        ...
        for (_rule_index, rule) in config.rules.iter().enumerate() {
            ...
            for (limiter_index, limiter_config) in rule.limiters.iter().enumerate() {
                let limiter = LimiterFactory::create(limiter_config).map_err(|e| {
                    FlowGuardError::ConfigError(format!(
                        "规则[{}]限流器[{}]创建失败: {}",
                        rule.id, limiter_index, e
                    ))
                })?;
    ...
    ///     let governor = /* ... */;
    ///     let context = RequestContext::new()
    ///         .with_header("X-API-Key", "user123");
    ///         .with_client_ip("192.168.1.1");
    ///
    ///     let result = governor.check_resource_parallel("user123").await?;
    ///     assert!(matches!(result, Ok(Decision::Allowed(None)));
    ///     assert!(!result.is_banned());
    /// }
    /// ```
    #[instrument(skip(self))]
    /// 使用专门的并行封禁检查器
    let parallel_checker = crate::parallel_ban_checker::ParallelBanChecker::new(self.ban_manager.clone());
        
        // 创建封禁目标列表
        let ban_targets = vec![
            BanTarget::UserId(resource.to_string()),
            BanTarget::Ip("127.0.0.1".to_string()),
            BanTarget::Mac("AA:BB:CC:DD:EE:FF".to_string()),
        ];
        
        // 并行检查所有目标
        let ban_info = parallel_checker.check_targets_parallel(&ban_targets, Some(&context)).await?;
        
        // 返回检查结果
        match ban_info {
            Some(info) => Ok(Decision::Banned(info)),
        ...
        for result in results {
            if let Some(ban_info) = result {
                return Ok(Decision::Banned(ban_info));
            }
        }
    ...
    pub async fn reset_stats(&self) {
        *self.stats.write().await = GovernorStats::default();
        ...
        let chain = self.decision_chain.write().await;
        chain.reset_stats();
        drop(chain);

        let matcher = self.rule_matcher.write().await;
        matcher.reset_stats();
        drop(matcher);
...
mod tests {
    use super::*;
    use crate::config::{GlobalConfig, LimiterConfig, Matcher as ConfigMatcher, Rule};
    ...
    async fn test_governor_check_resource() {
        ...
        // 使用简化接口检查资源
        let context = RequestContext::new().with_header("X-API-Key", "resource123");
        let decision = governor.check(&context).await.unwrap();
        assert_eq!(decision, Decision::Allowed(None));
    }

    #[tokio::test]
    async fn test_governor_check_resource_parallel() {
        let config = create_test_config();
        let storage = Arc::new(MemoryStorage::new());
        let ban_storage = Arc::new(crate::storage::MemoryStorage::new());
        let metrics = Arc::new(crate::telemetry::Metrics::new());
        ...
        let decision = governor.check_resource_parallel("test_user").await.unwrap();
        assert_eq!(decision, Decision::Allowed(None));
    }
